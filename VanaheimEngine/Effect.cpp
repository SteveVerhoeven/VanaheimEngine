...
void Effect::Initialize(ID3D11Device* pDevice, const std::string& assetFile)
{
	// ********************************
	// Load effect
	// ********************************
	m_pDirectXEffect = LoadEffect(pDevice, assetFile);
...
void Effect::Update(GameObject* pParentGameObject)
{
	...
	if (FAILED(hr))
		Locator::GetDebugLoggerService()->LogHRESULT(hr, __FILE__, std::to_string(__LINE__));
}
...
// ------------------------------------
ID3DX11Effect* Effect::LoadEffect(ID3D11Device* pDevice, const std::string& effectFilePath)
{
	HRESULT result = S_OK;
	...
	if (FAILED(result))
	{
		...
		// Log the error
		// -------------
		Locator::GetDebugLoggerService()->LogHRESULT(result, __FILE__, std::to_string(__LINE__));
...
ID3DX11EffectTechnique* Effect::BindTechniqueVariable(const std::string& techniqueName)
{
	// ********************************
	// Bind techniques
	// ********************************
...
ID3DX11EffectMatrixVariable* Effect::BindMatrixVariable(const std::string& matrixName)
{
	// ********************************
	// Bind matrix
	// ********************************
...
ID3DX11EffectShaderResourceVariable* Effect::BindTextureVariable(const std::string& textureName)
{
	// ********************************
	// Bind texture
	// ********************************
...
ID3DX11EffectRasterizerVariable* Effect::BindRasterizerStateVariable(const std::string& rasterizerName)
{
	// ********************************
...
void Effect::CreateSamplerStates(ID3D11Device* pDevice, const D3D11_FILTER& filter)
{
	...
	Locator::GetDebugLoggerService()->LogHRESULT(pDevice->CreateSamplerState(pSamplerStateDesc, ppSamplerState), __FILE__, std::to_string(__LINE__));
}